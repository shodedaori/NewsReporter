...
class GitHubLLMSummarizer:
    def __init__(
        self,
        prompt_system: str,
        prompt_user_template: str,
        config: GitHubLLMSummaryConfig,
    ...
    def summarize_item(self, item: Item) -> Item:
        ...
        description = strip_html(item.summary_raw)
        readme = strip_html(item.signals.get("readme_text", ""))
        content = readme if readme else description
        content = content[: self.config.max_input_chars]
        prompt_user = self.prompt_user_template.format(
            title=item.title,
            url=item.url,
            language=item.signals.get("language", ""),
            stars_today=item.signals.get("stars_today", 0),
            stars_total=item.signals.get("stars_total", item.signals.get("stars", 0)),
        ...
        try:
            ...
            LOGGER.warning("GitHub LLM summarize failed, fallback to extractive: %s", exc)
            return self.fallback.summarize_item(item)
...
def build_github_summarizer(app_config: dict, prompts_config: dict) -> Summarizer | GitHubLLMSummarizer:
    summarizer_config = app_config.get("summarizer", {})
    mode = str(summarizer_config.get("github_mode", summarizer_config.get("mode", "extractive"))).lower()
    fallback = Summarizer()
    if mode != "llm":
        return fallback
    ...
    config = GitHubLLMSummaryConfig(
        ...
        max_input_chars=int(llm_config.get("max_input_chars", 6000)),
        max_output_chars=int(llm_config.get("max_output_chars", 280)),
    )
    return GitHubLLMSummarizer(
        prompt_system=str(prompts_config.get("github_llm_summary_system", "")),
        prompt_user_template=str(prompts_config.get("github_llm_summary_user", "")),
        config=config,
        fallback=fallback,
    )
