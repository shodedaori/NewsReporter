...
class ArxivSectionPipeline(BaseSectionPipeline):
    section = "arxiv"
    ...
    def run(self, digest_date: date) -> SectionResult:
        print(f"[progress] section=arxiv start date={digest_date.isoformat()}", flush=True)
        ...
        for plugin in self.plugins:
            ...
            try:
                print(f"[progress] section=arxiv fetch plugin={plugin.name}", flush=True)
        ...
        print(
            f"[progress] section=arxiv dedup done total={len(all_items)} unique={len(unique_items)}",
            flush=True,
        )
        ...
        print(f"[progress] section=arxiv select top_items={len(selected_items)}", flush=True)
        summarized_items = self.summarizer.summarize_items(selected_items)
        print(f"[progress] section=arxiv summarize done items={len(summarized_items)}", flush=True)
        ...
        print(f"[progress] section=arxiv finished stats={stats}", flush=True)
