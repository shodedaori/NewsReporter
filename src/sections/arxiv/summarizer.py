...
class ArxivLLMSummarizer:
    def __init__(
        self,
        prompt_system: str,
        prompt_user_template: str,
        config: ArxivLLMSummaryConfig,
    ...
    def summarize_item(self, item: Item) -> Item:
        ...
        abstract = strip_html(item.summary_raw)[: self.config.max_input_chars]
        authors = ", ".join(item.signals.get("authors", []))
        categories = ", ".join(item.signals.get("categories", []))
        ...
        try:
            ...
            LOGGER.warning("arXiv LLM summarize failed, fallback to extractive: %s", exc)
            return self.fallback.summarize_item(item)
...
def build_arxiv_summarizer(app_config: dict, prompts_config: dict) -> Summarizer | ArxivLLMSummarizer:
    summarizer_config = app_config.get("summarizer", {})
    mode = str(summarizer_config.get("arxiv_mode", summarizer_config.get("mode", "extractive"))).lower()
    fallback = Summarizer()
    if mode != "llm":
        return fallback
    ...
    config = ArxivLLMSummaryConfig(
        ...
        max_output_chars=int(llm_config.get("max_output_chars", 320)),
    )
    return ArxivLLMSummarizer(
        prompt_system=str(prompts_config.get("arxiv_llm_summary_system", "")),
        prompt_user_template=str(prompts_config.get("arxiv_llm_summary_user", "")),
        config=config,
        fallback=fallback,
    )
