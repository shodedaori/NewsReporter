...
class HFLLMSummarizer:
    def __init__(
        self,
        prompt_system: str,
        prompt_user_template: str,
        config: HFLLMSummaryConfig,
    ...
    def summarize_item(self, item: Item) -> Item:
        ...
        summary_raw = strip_html(item.summary_raw)
        content = summary_raw[: self.config.max_input_chars]
        prompt_user = self.prompt_user_template.format(
            kind=item.signals.get("kind", ""),
            title=item.title,
            url=item.url,
            rank=item.signals.get("rank", ""),
            likes_7d=item.signals.get("likes_7d", 0),
            likes_total=item.signals.get("likes_total", 0),
            downloads=item.signals.get("downloads", 0),
            tags=", ".join(item.signals.get("tags", [])),
            pipeline_tag=item.signals.get("pipeline_tag", ""),
            license=item.signals.get("license", ""),
            language=item.signals.get("language", []),
            content=content,
        )
        try:
            ...
            LOGGER.warning("HF LLM summarize failed, fallback to extractive: %s", exc)
            return self.fallback.summarize_item(item)
...
def build_hf_summarizer(app_config: dict, prompts_config: dict) -> Summarizer | HFLLMSummarizer:
    summarizer_config = app_config.get("summarizer", {})
    mode = str(summarizer_config.get("hf_mode", summarizer_config.get("mode", "extractive"))).lower()
    fallback = Summarizer()
    if mode != "llm":
        return fallback
    ...
    config = HFLLMSummaryConfig(
        ...
        max_input_chars=int(llm_config.get("max_input_chars", 5000)),
        max_output_chars=int(llm_config.get("max_output_chars", 280)),
    )
    return HFLLMSummarizer(
        prompt_system=str(prompts_config.get("hf_llm_summary_system", "")),
        prompt_user_template=str(prompts_config.get("hf_llm_summary_user", "")),
        config=config,
        fallback=fallback,
    )
