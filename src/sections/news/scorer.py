...
class NewsScorer(BaseScorer):
    def __init__(self, scoring_config: dict):
        ...
        self.companies = [c.lower() for c in scoring_config.get("companies", [])]
    ...
    def score_item(self, item: Item, now: datetime) -> float:
        ...
        company_hits = sum(1 for company in self.companies if company in text)
        match_score = min(30.0, keyword_hits * 4.0 + company_hits * 5.0)
        ...
        item.signals["company_hits"] = company_hits
